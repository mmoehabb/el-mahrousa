...
export const moveOneStep = (state: GameState): GameState => {
  ...
  if (newPosition < player.position) {
    ...
    newState.logs = [`${player.name} passed GO and collected ${GAME_CONFIG.GO_REWARD} ${GAME_CONFIG.CURRENCY}`, ...newState.logs];
  } else {
    newPlayers[state.currentPlayerIndex] = {
      ...player,
      position: newPosition,
...
export const applyLandingLogic = (state: GameState): GameState => {
  ...
  if (tile.type === 'TAX') {
    ...
    newState.players = newPlayers;
    newState.logs = [`${player.name} paid ${taxAmount} ${GAME_CONFIG.CURRENCY} in tax`, ...newState.logs];
    ...
    if (owner && owner.id !== player.id && !owner.isBankrupt) {
      ...
      newState.players = newPlayers;
      newState.logs = [`${player.name} paid ${rent} ${GAME_CONFIG.CURRENCY} rent to ${owner.name}`, ...newState.logs];
      newState.turnPhase = 'END';
    }
...
const calculateRent = (tile: Tile, owner: Player, allTiles: Tile[]): number => {
  ...
  if (tile.type === 'AIRPORT') {
    const airportCount = allTiles.filter(t => t.type === 'AIRPORT' && owner.properties.includes(t.id)).length;
...
export const buyProperty = (state: GameState, tileId: number): GameState => {
  ...
  return {
    ...state,
    players: newPlayers,
    logs: [`${player.name} bought ${tile.name} for ${tile.price} ${GAME_CONFIG.CURRENCY}`, ...state.logs],
    turnPhase: 'END'
  };
};
...
export const executeTrade = (state: GameState, p1Id: string, p2Id: string, offer: { myCash: number, partnerCash: number, myProperties: number[], partnerProperties: number[] }): GameState => {
  const newPlayers = state.players.map(p => {
    if (p.id === p1Id) {
      return {
        ...p,
        balance: p.balance - offer.myCash + offer.partnerCash,
        properties: p.properties.filter((id: number) => !offer.myProperties.includes(id)).concat(offer.partnerProperties)
      };
    }
    if (p.id === p2Id) {
      return {
        ...
        properties: p.properties.filter((id: number) => !offer.partnerProperties.includes(id)).concat(offer.myProperties)
      };
    }
    return p;
  });
