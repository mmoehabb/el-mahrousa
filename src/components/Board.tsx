...
const Board: React.FC<BoardProps> = ({ handleRoll, isMyTurn, sendAction, onTileClick }) => {
  ...
  return (
    <div className="relative p-2 md:p-4 bg-egyptian-pattern rounded-lg shadow-2xl border-2 md:border-4 border-egyptian-gold w-[1200px] h-[1200px] min-w-[1200px] min-h-[1200px] max-w-none overflow-hidden">
      <div
      ...
      >
        ...
        {/* Center */}
        <div className="col-start-2 col-end-11 row-start-2 row-end-11 flex flex-col items-center justify-center bg-sand/20 dark:bg-slate-900/50 backdrop-blur-sm m-4 md:m-8 lg:m-12 border-2 md:border-4 border-egyptian-gold/40 rounded-lg relative p-8 space-y-6">
          <div className="flex gap-6 md:gap-8 bg-white/50 dark:bg-slate-800/80 p-6 rounded-3xl backdrop-blur-md border border-white/50 dark:border-slate-700/50 shadow-xl scale-100 sm:scale-125 md:scale-150">
            <motion.div
          ...
          <div className="w-full max-w-sm md:max-w-md space-y-4 md:mt-6">
            {isMyTurn && gameState.turnPhase === 'ROLL' && (
              <button
                onClick={handleRoll}
                className="w-full bg-egyptian-blue text-white py-4 md:py-5 rounded-2xl font-black flex items-center justify-center gap-2 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 text-base md:text-xl shadow-lg"
              >
                <Dice5 className="w-7 h-7 md:w-8 md:h-8" /> {t('game.rollDiceBtn')}
              </button>
            )}
            ...
            {isMyTurn && gameState.turnPhase === 'ACTION' && (
              <div className="space-y-4">
                {gameState.tiles[currentPlayer.position]?.price &&
                  !ownerByTile[currentPlayer.position] && (
                    <button
                      ...
                      className="w-full bg-green-600 text-white py-3 md:py-4 rounded-lg md:rounded-xl font-bold hover:bg-green-700 text-sm md:text-lg shadow-md"
                    >
                      {t('game.buyForBtn', {
                ...
                <button
                  ...
                  className="w-full bg-slate-500 text-white py-3 md:py-4 rounded-lg md:rounded-xl font-bold hover:bg-slate-600 text-sm md:text-lg shadow-md"
                >
            ...
            {isMyTurn && gameState.turnPhase === 'END' && (
              <button
                onClick={() => sendAction({ type: 'END_TURN' })}
                className="w-full bg-egyptian-blue text-white py-3 md:py-4 rounded-lg md:rounded-xl font-bold text-sm md:text-lg shadow-md"
              >
                {t('game.endTurnBtn')}
              </button>
            )}
            ...
            <div className="w-full mt-4 p-4 bg-white/40 dark:bg-slate-800/60 rounded-xl backdrop-blur-sm border border-white/30 dark:border-slate-600/30 text-start font-bold flex flex-col items-start overflow-y-auto max-h-48 md:max-h-56 hide-scrollbar shadow-inner">
              {recentLogs.length > 0 ? (
