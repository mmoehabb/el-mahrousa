...
_In this Authoritative Host Architecture, the player who creates the lobby acts as the Host. Peers send their intents (actions) to the Host. The Host processes the actions through `gameLogic`, updates the `GameContext`, and broadcasts the new state back to all Peers._
...
_This starts a signaling server on `ws://localhost:9000`._
